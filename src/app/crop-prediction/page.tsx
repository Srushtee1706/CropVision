...
import CropPredictionForm, {
  formSchema,
} from "../../components/CropPredictionForm";
...
export default function CropPredictionPage() {
  ...
  const [predictionData2, setPredictionData2] =
    useState<PredictionData2 | null>(null);
  ...
  return (
    <main className="min-h-screen bg-gradient-to-br from-green-50 to-green-100 flex w-full flex-col items-center p-6">
      ...
      {currentStep === "form" && (
        <CropPredictionForm onSubmit={handleFormSubmit} />
      )}

      {currentStep === "loading" && (
        <LoadingAnimation cropType={formData?.crop || "crop"} />
      )}
